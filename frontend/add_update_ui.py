...
def add_update_tab():
    ...
    with st.form(key="expenses-form"):
        # Responsive header - hide on mobile, show on desktop
        ...
        for i in range(st.session_state.num_expense_rows):
            ...
            with col1:
                amount_input = st.number_input(
                    "Amount" if st.session_state.get('screen_width', 1000) <= 768 else "Amount",
                    min_value=0.0,
                    step=1.0,
                    value=amount,
                    key=f"amount_{i}_{selected_date}",
                    label_visibility="visible" if st.session_state.get('screen_width', 1000) <= 768 else "collapsed"
                )
            with col2:
                category_input = st.selectbox(
                    label="Category" if st.session_state.get('screen_width', 1000) <= 768 else "Category",
                    options=categories,
            ...
            with col3:
                notes_input = st.text_input(
                    label="Notes" if st.session_state.get('screen_width', 1000) <= 768 else "Notes",
                    value=notes,
    ...
    # Confirmation dialog outside the form
        ...
        with col1:
            if st.button("✅ Yes, Submit", type="primary", use_container_width=True):
                response = requests.post(f"{API_URL}/expenses/{selected_date}", json=filtered_expenses,
                                         headers=get_headers())
                if response.status_code == 200:
                    st.success("✅ Successfully submitted expenses!")
                    time.sleep(1.5)
                    # Reset everything
                    st.session_state.num_expense_rows = 5
                    st.session_state.rows_to_delete = set()
